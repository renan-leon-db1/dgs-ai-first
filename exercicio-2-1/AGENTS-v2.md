...
- DEVE validar toda entrada externa (HTTP body, parâmetros de query, variáveis de ambiente) com schema Zod antes de usar qualquer campo.
- DEVE validar a saída do LLM com schema Zod antes de retornar ao cliente.
- DEVE definir todos os schemas exclusivamente em src/shared/schemas/ e reutilizá-los entre functions e services.
- NUNCA criar schema ou validator local em src/functions/ ou src/services/ (ex.: validator.ts com z.object local é proibido).
- DEVE, quando um schema ainda não existir, criá-lo primeiro em src/shared/schemas/ e só então importá-lo via alias @/shared/schemas/... no handler/service.
- FAÇA (correto, inline): import { QueryPayloadSchema } from @/shared/schemas/query; const body = QueryPayloadSchema.parse(await req.json());
- NÃO FAÇA (incorreto, inline): const QueryPayloadSchema = z.object({ query: z.string() }); // definido dentro de validator.ts

```typescript
// ✅ FAÇA — schema compartilhado em src/shared/schemas/, reutilizado no handler
...
// ❌ NÃO FAÇA — schema local na function (violação)
import { z } from 'zod';
...
- DEVE centralizar helpers de montagem de entrada/contexto de teste (ex.: `buildRequest`, `buildContext`) exclusivamente em `tests/fixtures/` para reutilização entre suítes.
- NUNCA declarar helpers locais no próprio arquivo de teste quando o helper puder ser reutilizado por outra suíte.
- DEVE centralizar mocks de clientes Azure (OpenAI, AI Search) exclusivamente em `tests/fixtures/azure-mocks.ts` e registrá-los via `vi.mock` no teste.
- NUNCA declarar `vi.fn()` inline dentro de arquivo de teste para dependências Azure (ex.: `const searchRelevantChunksMock = vi.fn()` ou `const completeQueryMock = vi.fn()`).
- DEVE, quando um mock ainda não existir, criá-lo primeiro em `tests/fixtures/azure-mocks.ts` e só então importá-lo no teste.
- DEVE, quando um helper ainda não existir, criá-lo primeiro em `tests/fixtures/` e só então importá-lo no teste.
- FAÇA (correto, inline): `import { buildRequest, buildContext } from '@/tests/fixtures/query-context'; const req = buildRequest({ query: 'prazo de entrega' });`
- NÃO FAÇA (incorreto, inline): `function buildRequest() { ... } function buildContext() { ... } // dentro do arquivo de teste`
- FAÇA (correto, inline): `import { searchRelevantChunksMock, completeQueryMock } from '@/tests/fixtures/azure-mocks'; vi.mock('@/services/search', () => ({ searchRelevantChunks: searchRelevantChunksMock }));`
- NÃO FAÇA (incorreto, inline): `const searchRelevantChunksMock = vi.fn(); const completeQueryMock = vi.fn();`

```typescript
// ✅ FAÇA — importe helpers e mocks centralizados de tests/fixtures/
import { buildRequest, buildContext } from '@/tests/fixtures/query-context';
import {
  searchRelevantChunksMock,
  completeQueryMock
} from '@/tests/fixtures/azure-mocks';
import { describe, it, expect, vi } from 'vitest';

const req = buildRequest({ query: 'prazo de entrega' });
const ctx = buildContext({ userId: 'anon-user' });

vi.mock('@/services/search', () => ({
  searchRelevantChunks: searchRelevantChunksMock
}));

vi.mock('@/services/completion', () => ({
  completeQuery: completeQueryMock
}));
...
// ❌ NÃO FAÇA — helpers locais no arquivo de teste (violação)
function buildRequest() {
  return { body: { query: 'prazo de entrega' } };
}

function buildContext() {
  return { invocationId: 'local-context' };
}

// ❌ NÃO FAÇA — mocks inline no arquivo de teste (violação)
const searchRelevantChunksMock = vi.fn();
const completeQueryMock = vi.fn();
```
